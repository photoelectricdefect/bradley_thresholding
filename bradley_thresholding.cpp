...
void bradley_thresholding::threshold(const cv::Mat& im,cv::Mat& out,int winr,int t) {
    ...
    for(size_t x=0;x<im.cols;x++) {
        for(size_t y=0;y<im.rows;y++) {
            ...
            double mean=sat.mean(x,y,winr);

            if(predicted<=mean*(100-t)/100) out.ptr<uint8_t>(y)[x]=0;
